...
def train_one_epoch(
    ...
    # Train
    model.train()
    for i, (us_path, clinic_info, label) in enumerate(train_loader):
        assert not torch.isnan(us_path).any(), "Image contains NaN!"
        assert not torch.isinf(us_path).any(), "Image contains Inf!"
        assert not torch.isnan(clinic_info).any(), "Clinical contains NaN!"
        assert not torch.isinf(clinic_info).any(), "Clinical contains Inf!"

        # Only use one US image (us_path)
        logist = model(us_path, clinic_info=clinic_info)  # Pass us_path instead of multiple US images
        ...
        step += 1
        accelerator.print(
            f"Epoch [{epoch + 1}/{config.bmu.num_epochs}] Training [{i}/{len(train_loader)}] Train Loss: {loss_category:1.5f} "
        )
...
def val_one_epoch(
    ...
    # Eval
    model.eval()
    for i, (us_path, clinic_info, label) in enumerate(val_loader):
        # Only use one US image (us_path) and no mg images
        logist = model(us_path, clinic_info=clinic_info)  # Pass only us_path
...
if __name__ == "__main__":
    ...
    # ------------------------不使用预训练权重start----------------------
    # No pretrained weights are loaded for model
    # ------------------------不使用预训练权重end----------------------
